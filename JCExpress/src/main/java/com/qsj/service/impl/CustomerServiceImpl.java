...
public class CustomerServiceImpl implements CustomerService {
	 ...
	 */
	@Override
	public OsCustomer checkCustomer(String nickname) {
		return customerMapper.findCustomerByUsername(nickname);
	}
	
	@Override
	public ExOsResult customerLogin(String customerCode, String password) {
		System.out.println("customerCode==="+customerCode+"  password==="+password);//TODO log
		List<OsCustomer> osCustomer = customerMapper.findCustomerByEmailOrTelephone(customerCode);
		System.out.println("osCustomer==="+osCustomer);//TODO log
		...
		redisTemplate.opsForValue().set(sessionKeyPrefix + token, 
				JSONObject.fromObject(customer).toString(), 30, TimeUnit.MINUTES);
	...
	@Override
	public PageResult findAllCustomer(String search, BasePage basePage) {
		List<OsCustomer> list = customerMapper.findAllCustomerByPage(search, basePage);
		return new PageResult(list, basePage.getTotal());
	}
}

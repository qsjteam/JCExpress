    ...
    <div class="col-sm-12">
      <div class="ibox float-e-margins">
        <div class="ibox-title">
         <h5>修改管理员<small><b>用户个人信息时应当遵循合法、正当、必要的原则，明示目的、方式和范围，并经用户同意<big>♂</big></b></small></h5>
          <div class="ibox-tools"> <a class="collapse-link"><i class="fa fa-chevron-up"></i></a> <a class="close-link"><i class="fa fa-times"></i></a> </div>
        </div>
        <div class="ibox-content">
          <form id="form" class="form-horizontal" action="${mao}/user/list/${user.u_id}" data-method="put">
          <input type="hidden" name="_method" value="put" />
            <div class="form-group">
              <label class="col-sm-2 col-xs-offset-1 control-label">用户名：</label>
              <div class="col-sm-6">
                <input type="text" maxlength="11" class="form-control" name="login_name" value="${user.login_name}">
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-6">
                <input type="text" maxlength="11" class="form-control" name="user_name" value="${user.user_name}">
              </div>
            </div>
            <div class="hr-line-dashed"></div>
            <div class="form-group">
              ...
              <div class="col-sm-9">
                <c:forEach items="${roles}" var="role">
                  <div class="checkbox col-sm-4">
                    <label>
                      <input type="checkbox" name="roleId" value="${role.role_id}" <c:forEach items="${userRoles}" var="userRole">${role.role_id eq userRole.role_id ?'checked="checked"':''}</c:forEach>/>
                      &nbsp;${role.role_name} 
                    </label>
                  </div>
                </c:forEach>
              ...
              <div class="col-sm-6">
                <select class="form-control" name="organization_id">
                  ...
                  <c:forEach items="${organizations }" var="organization">
                    <option value="${organization.org_id }" ${user.organization_id eq organization.org_id ?'selected="selected"':''}>${organization.org_name }</option>
                  </c:forEach>
                </select>
              </div>
            </div>

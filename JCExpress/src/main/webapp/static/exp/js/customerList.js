...
function nameFormatter(value, row) {
	return '<a href="javascript:void(0)" onclick="layer_show(\'' + row.login_name + '\',\'' + baselocation + '/customer/list/'+ row.u_id+'/userMessage' + '\',\'530\',\'625\')">' + value + '</a>';
}
...
window.actionEvents = {
	...
	'click .edit' : function(e, value, row, index) {
		layer_show(row.login_name, baselocation + '/customer/list/'+ row.u_id+"/updateUser",830, 550)
	},
	'click .remove' : function(e, value, row, index) {
		admin_delete(index, row.u_id);
	},
	'click .log' : function(e, value, row, index) {
		layer_show(row.user_name + '登录日志', baselocation + '/customer/list/'+ row.u_id+'/userLog', 900, 650)
	}
};

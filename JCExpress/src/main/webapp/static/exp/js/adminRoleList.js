...
window.actionEvents = {
	'click .freeze' : function(e, value, row, index) {
		status_stop(index, row.role_id);
	},
	'click .normal' : function(e, value, row, index) {
		status_start(index, row.role_id);
	},
	'click .edit' : function(e, value, row, index) {
		layer_show(row.role_name, baselocation + '/role/list/'+row.role_id+'/updateRole', 1100, 650)
	},
	'click .remove' : function(e, value, row, index) {
		admin_delete(index, row.role_id);
	},
	'click .log' : function(e, value, row, index) {
		layer_show(row.role_name, baselocation + '/role/list/'+ row.role_id+'/roleUser', 1100, 650)
	}
};
...
function status_stop(index, value) {
	layer.confirm('确认要冻结该角色吗？', {
		...
		$.ajax({
			dataType : 'json',
			type : 'put',
			url : baselocation + '/role/list/'+ value+'/frozenRole',
...
function admin_delete(index, value) {
	layer.confirm('确认要删除该角色吗？', {
		...
		$.ajax({
			type : 'delete',
			dataType : 'json',
			url : baselocation + '/role/list/' + value,
			success : function(result) {
				if (result.code == 1) {

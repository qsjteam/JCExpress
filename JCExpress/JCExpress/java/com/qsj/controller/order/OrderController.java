 ...
 * 
 * @Package: org.mao.controller.order 
 * @author:作者 Mao   
 * @date:创建时间 2018年2月16日 下午3:10:48 
 */
...
public class OrderController {
	...
	@ResponseBody
	private Object orderList(BasePage page, @RequestParam(required = false, value = "search") String search) {
	...
	private String orderDetail(@PathVariable("orderId") String orderId, @PathVariable("status") String status,
			Model model) {
	...
	private String orderAudit(Model model) {
	...
	@ResponseBody
	private Object orderAudit(BasePage page, @RequestParam(required = false, value = "search") String search) {
		User activeUser = (User) SecurityUtils.getSubject().getPrincipal();
		if (activeUser != null) {
	...
	private String updateOrder(@PathVariable("orderId") String orderId, Model model) {
	...
	@ResponseBody
	private Object updateOrder(@PathVariable("orderId") String orderId, ExOrder exOrder, ExOrderDetail exOrderDetail) {
		User activeUser = (User) SecurityUtils.getSubject().getPrincipal();
		if (activeUser != null) {
	...
	@ResponseBody
	private Object orderAudit(@PathVariable("orderId") String orderId) {
		User activeUser = (User) SecurityUtils.getSubject().getPrincipal();
		if (activeUser != null) {
	...
	@ResponseBody
	private Object createOrder(ExOrder exOrder, ExOrderDetail exOrderDetail) {
	...
	@ResponseBody
	private Object createOrders(ExOrder exOrder, String receives) {
